	...
	massDelete () {
		...
		this.setState({
			confirmationDialog: {
				isOpen: true,
				label: 'Удалить',
				body: `Вы действительно хотите удалить ${itemCount}?<br /><br />Данное действие невозможно отменить.`,
				onConfirmation: () => {
	...
	renderSearch () {
		...
		return (
			<InputGroup.Section grow className="ListHeader__search">
				<FormInput ref="listSearchInput" value={this.state.searchString} onChange={this.updateSearch} onKeyUp={this.handleSearchKey} placeholder="Поиск" className="ListHeader__searchbar-input" />
	...
	deleteTableItem (item, e) {
		...
		this.setState({
			confirmationDialog: {
				isOpen: true,
				label: 'Удалить',
				body: `Вы действительно хотите удалить <strong>${item.name}</strong>?<br /><br />Данное действие невозможно отменить.`,
				onConfirmation: () => {
					this.props.dispatch(deleteItem(item.id));
					this.removeConfirmationDialog();
				},
