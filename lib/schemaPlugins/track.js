	...
	_.forEach(options, function (value, key) {
		...
		if (_.has(defaultOptions, key)) {
			...
			if (value) {
				...
				switch (key) {
					case 'createdAt':
						fields[fieldName] = { label: 'Дата создания', type: Date, hidden: true, index: true };
						break;

					case 'updatedAt':
						fields[fieldName] = { label: 'Дата обновления', type: Date, hidden: true, index: true };
						break;

					case 'createdBy':
						fields[fieldName] = { label: 'Кем создано', type: Types.Relationship, ref: userModel, hidden: true, index: true };
						break;

					case 'updatedBy':
						fields[fieldName] = { label: 'Кем обновлено', type: Types.Relationship, ref: userModel, hidden: true, index: true };
						break;
				}
			}
		} else {
